...
{
	unsigned char *tem_ptr = (unsigned char *) ((unsigned char *)msg - sizeof (ble_hdr));	
	
	ble_msg *blemsg = (ble_msg *) tem_ptr; //((unsigned char *)msg - sizeof (ble_hdr));		
	...
	if (app_env.slave_on_sleep == SLAVE_ACTIVE){
		...
		EnQueue(&SPITxQueue, tem_ptr,(unsigned short)(blemsg->bLength + sizeof(ble_hdr)));
