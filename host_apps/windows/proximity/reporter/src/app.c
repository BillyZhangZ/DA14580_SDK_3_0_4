...
{
    struct gapc_get_info_cmd * req = BleMsgAlloc(GAPC_GET_INFO_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device.device.conidx), TASK_APP,
...
{
    struct gapc_disconnect_cmd * req = BleMsgAlloc(GAPC_DISCONNECT_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device.device.conidx), TASK_APP,
...
void app_security_enable(void)
{
   ...
   struct gapc_bond_cmd * req = BleMsgAlloc(GAPC_BOND_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device.device.conidx), TASK_APP,
...
{
    // confirm connection
    struct gapc_connection_cfm *cfm = BleMsgAlloc(GAPC_CONNECTION_CFM, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device.device.conidx), TASK_APP,
...
{
    struct gapc_bond_cfm * cfm = BleMsgAlloc(GAPC_BOND_CFM, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device.device.conidx), TASK_APP,
...
{
    // Allocate the message
    struct gapc_encrypt_cmd * req = BleMsgAlloc(GAPC_ENCRYPT_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device.device.conidx), TASK_APP,
...
void app_proxr_enable(void)
{
    ...
    printf("Sending PROXR_ENABLE_REQ\n");
...
void app_dis_enable(void)
{
    ...
    printf("Sending DISS_ENABLE_REQ \n");
...
void app_proxr_rd_tx_pwr(void)
{
    ...
    // Allocate the message
    struct llc_rd_tx_pw_lvl_cmd * req =  BleMsgAlloc(LLC_RD_TX_PW_LVL_CMD, TASK_LLC, TASK_APP,
                                                     sizeof(struct llc_rd_tx_pw_lvl_cmd));
        
    req->conhdl = app_env.proxr_device.device.conhdl;
    req->type = TX_LVL_CURRENT;

    printf ("\t Sending Msg  LLC_RD_TX_PW_LVL_CMD \n");
