...
{
    struct gapm_reset_cmd *msg = BleMsgAlloc(GAPM_RESET_CMD, TASK_GAPM, TASK_APP,
...
void app_inq(void)
{
    struct gapm_start_scan_cmd *msg = BleMsgAlloc(GAPM_START_SCAN_CMD, TASK_GAPM, TASK_APP,
...
void app_connect(unsigned char indx)
{
    ...
    msg = (struct gapm_start_connection_cmd *) BleMsgAlloc(GAPM_START_CONNECTION_CMD , TASK_GAPM, TASK_APP,
...
void app_read_rssi(void)
{
	...
	for (i=0;i<MAX_CONN_NUMBER;i++){
		if (app_env.proxr_device[i].isactive == true){
			struct gapc_get_info_cmd * req = BleMsgAlloc(GAPC_GET_INFO_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device[i].device.conidx), TASK_APP,
...
void app_disconnect(int con_id)
{
	...
	struct gapc_disconnect_cmd *req = BleMsgAlloc(GAPC_DISCONNECT_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
void app_security_enable(int con_id)
{
    ...
    struct gapc_bond_cmd * req = BleMsgAlloc(GAPC_BOND_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{
    // confirm connection
    struct gapc_connection_cfm *cfm = BleMsgAlloc(GAPC_CONNECTION_CFM, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{
    struct gapc_bond_cfm * req = BleMsgAlloc(GAPC_BOND_CFM, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{
    // Allocate the message
    struct gapc_encrypt_cmd * req = BleMsgAlloc(GAPC_ENCRYPT_CMD, KE_BUILD_ID(TASK_GAPC, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
void app_proxm_enable(int con_id)
{
    ...
    // Allocate the message
    struct proxm_enable_req * req = BleMsgAlloc(PROXM_ENABLE_REQ, KE_BUILD_ID(TASK_PROXM, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{   
    struct proxm_rd_alert_lvl_req * req = BleMsgAlloc(PROXM_RD_ALERT_LVL_REQ,  KE_BUILD_ID(TASK_PROXM, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{
    struct proxm_rd_txpw_lvl_req * req = BleMsgAlloc(PROXM_RD_TXPW_LVL_REQ, KE_BUILD_ID(TASK_PROXM, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{   
    struct proxm_wr_alert_lvl_req * req = BleMsgAlloc(PROXM_WR_ALERT_LVL_REQ,  KE_BUILD_ID(TASK_PROXM, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
{       
    // Allocate the message
    struct proxm_enable_req * req = BleMsgAlloc(DISC_ENABLE_REQ, TASK_DISC, TASK_APP,
                                                sizeof(struct proxm_enable_req));
...
{   
	struct disc_rd_char_req * req = BleMsgAlloc(DISC_RD_CHAR_REQ, KE_BUILD_ID(TASK_DISC, app_env.proxr_device[con_id].device.conidx), TASK_APP,
...
void app_cancel(void)
{
    ...
    msg = (struct gapm_cancel_cmd *) BleMsgAlloc(GAPM_CANCEL_CMD, TASK_GAPM, TASK_APP,
...
{
    struct gapm_cancel_cmd *msg = BleMsgAlloc(GAPM_CANCEL_CMD, TASK_GAPM, TASK_APP,
                                             sizeof(struct gapm_cancel_cmd));
