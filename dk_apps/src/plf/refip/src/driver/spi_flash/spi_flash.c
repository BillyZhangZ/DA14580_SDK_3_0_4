...
int32_t spi_flash_configure_memory_protection(uint8_t spi_flash_memory_protection_setting)
{
    if (spi_flash_set_write_enable() != ERR_OK)         // send [Write Enable] instruction
		return ERR_TIMEOUT;   
    return spi_flash_write_status_reg((spi_flash_read_status_reg() & (~W25x_MEM_PROT_BITMASK)) | ((spi_flash_memory_protection_setting<<2)&(W25x_MEM_PROT_BITMASK)));        
}

/**
 ****************************************************************************************
 * @brief Erase chip, removing all memory areas protection prior to erasure
 * @param forced: All memory protection settings are removed prior to
 *                chip erasure, provided the IC is recognized by the Manuf/Device Id. 
 * @return error code or success (ERR_OK)
 ****************************************************************************************
 */
int8_t spi_flash_chip_erase_forced(void)
{
    ...
    // All memory protection settings are removed prior to chip erasure
    man_and_dev_id = spi_read_flash_memory_man_and_dev_id() & 0xFFFF;
    switch (man_and_dev_id)
    {
        case W25X10_MAN_DEV_ID: //1MBit
            spi_flash_configure_memory_protection(W25x10_MEM_PROT_NONE);
            break;            
        case W25X20_MAN_DEV_ID: //2MBit
            spi_flash_configure_memory_protection(W25x20_MEM_PROT_NONE);
            break;        
        default:
