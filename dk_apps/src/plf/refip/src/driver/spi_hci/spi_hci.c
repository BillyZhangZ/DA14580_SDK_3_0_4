...
static void spi_transmit_isr(void)
{
    ...
    do {
        spi_rxtxreg_write(0x00);      // Write 0x00 byte for the next SPI transaction
        ...
        if(tmp != DREADY_ACK) {       // If DREADY is not acknowledged,
            spi_dready_low();         // de-assert DREADY and wait for closed CS
            while(!spi_cs_getf());    // Polling CS to detect if data is being received
        }
...
/**
 ****************************************************************************************
 * @brief Set SPI flow on or off by sending a flow on or flow off byte respectively.
 *
 * Asserts DREADY to request transmission and transmits the flow on or flow off byte.
 *
 * @param[in]      on   0 to set flow off, 1 to set flow on.
 ****************************************************************************************
 */
void spi_hci_set_flow(uint8_t on)
{
    uint8_t tmp;
    ...
    do {                              
        ...
        tmp = spi_rxtxreg_read();     // Get byte from SPI
        spi_int_bit_clear();          // Clear pending flag
        if(tmp != DREADY_ACK) {       // If DREADY is not acknowledged, de-assert DREADY 
            spi_dready_low();         //     and wait for closed CS
            while(!spi_cs_getf());    // Polling CS to detect if data is being received
        }
    } while (tmp != DREADY_ACK);      // If DREADY is not acknowledged, try again
...
void spi_hci_write_func(uint8_t *bufptr, uint32_t size, void (*callback) (uint8_t))
{
    ...
    spi_hci_flow_off_func();    // Send flow off byte
