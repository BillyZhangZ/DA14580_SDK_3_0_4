...
/******************************************************************************************/
	...
	/// HPS Profile Server Role
    #if defined(CFG_PRF_HPSS)
        #define BLE_HPS_SERVER               1
    #else
        #define BLE_HPS_SERVER               0
    #endif /* defined(CFG_PRF_HPSS)) */

   	/// HPS Profile Client Role
    #if defined(CFG_PRF_HPSC)
        #define BLE_HPS_CLIENT               1
    #else
        #define BLE_HPS_CLIENT               0
    #endif /* defined(CFG_PRF_HPSC)) */
    ...
    #define BLE_PAS_SERVER              0
	/// HTTP Proxy Server Role
	#define BLE_HPS_SERVER			    0
	/// HTTP Proxy Client Role
	#define BLE_HPS_CLIENT			    0
...
/// BLE_CLIENT_PRF indicates if at least one client profile is present
    ...
    || BLE_PAS_CLIENT || BLE_STREAMDATA_HOST || BLE_HPS_CLIENT)
...
/// BLE_SERVER_PRF indicates if at least one server profile is present
    ...
    || BLE_PAS_SERVER || BLE_SAMPLE128 || BLE_HPS_SERVER)
