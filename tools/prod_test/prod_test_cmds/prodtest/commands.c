...
static int parse_data_length(int *return_status, const char * str)
{
	...
	*return_status = 0;
	...
	// check: an number between 0 and 37
	if (result < 0 || 37 < result )
		*return_status = 1;
...
int starttest_tx_param_len_3_handler(int argc, char **argv)
{
	...
	Num_HCI_Command_Packets = evt->parameters[0];
	opcode = evt->parameters[1] + 256 * evt->parameters[2];
...
int starttest_tx_param_len_5_handler(int argc, char **argv)
{
	...
	evt2 = hci_recv_event_wait(60000); // wait for 60 seconds
	...
	handle_hci_event(evt2); ////////////////////////////////////////////// print evt

	if ( !( evt2->event == 0x0E 
		&& evt2->length == 3
		&& evt2->parameters[1] == 0x40
		&& evt2->parameters[2] == 0x40)
	)
	{
		return_status = SC_UNEXPECTED_EVENT; // unexpected event
		goto exit_command_handler;
...
int starttest_rx_readback_values_handler(int argc, char **argv)
{
	...
	if ( !( evt->event == 0x0E 
		&& evt->length == 3
		&& evt->parameters[1] == 0x20
		&& evt->parameters[2] == 0x40)
	)
	{
		return_status = SC_UNEXPECTED_EVENT; // unexpected event
		goto exit_command_handler;
...
int xtal_trimming_cmd_handler(int argc, char **argv)
{
    ...
    int trim_value_or_delta_arg_required = 0;
...
int otp_read_cmd_handler(int argc, char **argv)
{
    ...
    if (return_status != 0
        ...
        || (otp_address + word_count * 4 > 0x8000) )
    {
