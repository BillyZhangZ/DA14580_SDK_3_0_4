...
static void usage(const char* my_name)
{
	fprintf(stderr,
		...
		"Usage: %s in_file version_file out_file\n"
		"\n"
...
static int append_file(int outf, int inf, uint32_t* csum)
{
	ssize_t n;
	uint32_t crc32 = ~0;
...
int main(int argc, const char* argv[])
{
	...
	store32(hdr.code_size, sbuf.st_size);
